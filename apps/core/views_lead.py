...
class LeadDashboardView(LeadRequiredMixin, ListView):
    ...
    def get(self, request):
        return render(request, self.template_name, {
            ...
            'member_count': ClubMembership.objects.filter(role=ClubMembership.Role.MEMBER, is_active=True).count(),
            ...
            ).select_related('author', 'wing', 'category').order_by('-created_at')[:6],
            'upcoming_events': Event.objects.filter(is_public=True).count(),
            ...
            'users': User.objects.select_related('profile', 'club_membership').order_by('-date_joined')[:8],
        })


class LeadMembersView(LeadRequiredMixin, ManagedUsersListMixin, ListView):
...
def lead_add_user(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            messages.success(request, f'Created {user.display_name} successfully.')
            return redirect('lead:members')
    else:
        form = RegisterForm()
    return render(request, 'dashboard/lead/user_form.html', {'form': form, 'title': 'Add User'})
...
def lead_approve_member(request, pk):
    return approve_managed_user(request, pk, members_url='lead:members')
...
def lead_delete_user(request, pk):
    ...
    if user.pk == request.user.pk:
        ...
        return redirect('lead:members')
    user.delete()
    log_audit(request, 'lead_deleted_user', 'User', pk)
...
def lead_announcement_create(request):
    ...
    return render(request, 'dashboard/lead/announcement_form.html', {'form': form, 'title': 'Create Announcement'})
...
def lead_announcement_edit(request, pk):
    ...
    return render(request, 'dashboard/lead/announcement_form.html', {'form': form, 'title': 'Edit Announcement', 'announcement': announcement})
...
class LeadAnnouncementsListView(LeadRequiredMixin, ListView):
    model = Announcement
...
class LeadCompletedEventsView(LeadRequiredMixin, ListView):
    model = Event
    template_name = 'dashboard/lead/events.html'
    context_object_name = 'events'

    def get_queryset(self):
        return Event.objects.filter(status=Event.Status.COMPLETED).order_by('-date')

    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['completed_events'] = ctx['events']
        ctx['events'] = Event.objects.filter(status__in=[
...
class LeadGalleryView(LeadRequiredMixin, ListView):
    model = GalleryItem
...
def lead_gallery_edit(request, pk):
    ...
    return render(request, 'dashboard/lead/gallery_form.html', {'form': form, 'title': 'Edit Photo', 'item': item})


class LeadGalleryUploadView(LeadRequiredMixin, ListView):
    template_name = 'dashboard/lead/gallery_form.html'

    def get(self, request):
        return render(request, self.template_name, {'form': GalleryItemForm(), 'title': 'Upload Photo'})
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            return redirect('lead:gallery')
        return render(request, self.template_name, {'form': form, 'title': 'Upload Photo'})


class LeadContactMessagesView(LeadRequiredMixin, ListView):
